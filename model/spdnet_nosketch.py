...
class Rainnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.sub1(x_init)  # (4,32,256,256)
        # 先经过了这层SRiR
        ...
        x2 = self.sub2(self.ag1(torch.cat((x1, x_init), dim=1)))  # (4,32,256,256)
        ...
        x2 = x2.to('cuda:3')
        x3 = self.sub3(self.ag2(torch.cat((x2, x1, x_init), dim=1)))  # + x2 # 3
        x3 = self.ag_en(torch.cat([x3, x2, x1], dim=1))
        out3 = self.output3(x3)

        return out1.to('cuda:1'), out2.to('cuda:1'), out3.to('cuda:1')
