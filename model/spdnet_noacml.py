...
class subnet(nn.Module):
    def __init__(self, n_feats, blocks=2):
        ...
        self.AWmix2_1 = RIR(n_feats=n_feats, n_blocks=blocks)
        ...
        self.AWmix2_2 = RIR(n_feats=n_feats, n_blocks=blocks)
        ...
        self.AWmix2_3 = RIR(n_feats=n_feats, n_blocks=blocks)
...
class Rainnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.sub2(self.ag1(torch.cat((x1, x_init), dim=1)),
                       self.res_extra2(torch.cat((res_out1, res_out1, res_out1), dim=1)))  # (4,32,256,256)
        ...
        return res_x.to('cuda:1'), res_out1.to('cuda:1'), res_out2.to('cuda:1'), out1.to('cuda:1'), out2.to(
            'cuda:1'), out3.to('cuda:1')
