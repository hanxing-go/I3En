...
def main():
    ...
    #     res1 = res_x
    #     # 将增亮后的图像从Tensor转换为PIL Image
    #     res1 = res1.squeeze().detach().cpu().numpy()
    #     res1 = np.transpose(res1, (1, 2, 0))
    #     res1 = (res1 * 255).clip(0, 255).astype('uint8')
    #     res1 = Image.fromarray(res1)
    #
    #     res2 = res_out1
    #     # 将增亮后的图像从Tensor转换为PIL Image
    #     res2 = res2.squeeze().detach().cpu().numpy()
    #     res2 = np.transpose(res2, (1, 2, 0))
    #     res2 = (res2 * 255).clip(0, 255).astype('uint8')
    #     res2 = Image.fromarray(res2)
    #
    #     res3 = res_out2
    #     # 将增亮后的图像从Tensor转换为PIL Image
    #     res3 = res3.squeeze().detach().cpu().numpy()
    #     res3 = np.transpose(res3, (1, 2, 0))
    #     res3 = (res3 * 255).clip(0, 255).astype('uint8')
    #     res3 = Image.fromarray(res3)
    #
    #     save_path = '/mnt/jxsd_jaw/motongstudio/zx/LOLv1_test/enhance3/'
    #     # 保存增亮后的图像到本地文件
    ...
    #
    #     save_path = '/mnt/jxsd_jaw/motongstudio/zx/LOLv1_test/sketch3/'
    #     # 保存增亮后的图像到本地文件
    #     image_name = 'sketch_res_1%d' % iteration
    #     u_name = '%s.png' % (image_name)
    #     u_path = save_path + '/' + u_name
    #     res1.save(u_path)
    #
    #     # 保存增亮后的图像到本地文件
    #     image_name = 'sketch_res_2%d' % iteration
    #     u_name = '%s.png' % (image_name)
    #     u_path = save_path + '/' + u_name
    #     res2.save(u_path)
    #
    #     # 保存增亮后的图像到本地文件
    #     image_name = 'sketch_res_3%d' % iteration
    #     u_name = '%s.png' % (image_name)
    #     u_path = save_path + '/' + u_name
    #     res3.save(u_path)
    #
    ...
    for epoch in range(args.epochs):
        ...
        for iteration, (img_lowlight,img_highlight,sketch) in enumerate(train_queue):
            ...
            res_x,res_out1,res_out2,enhanced_image1,enhanced_image2,enhanced_image3 = my_model(img_lowlight,1)
            # res_x,res_out1,enhanced_image2,enhanced_image3= my_model(img_lowlight,1)
            ...
            L_Hist3=l_hist(enhanced_image3,img_highlight)
        ...
        torchvision.utils.save_image(torch.cat([img_lowlight[0],enhanced_image2[0],enhanced_image3[0],img_highlight[0]], dim=2),
                                                 args.sample_dir + str(epoch) + '.png')
        # torchvision.utils.save_image(torch.cat([res_x[0],res_out1[0], res_out2[0],sketch3[0]], dim=2),
        #                                          args.sample_dir + str(epoch) +'sketch3'+ '.png')
